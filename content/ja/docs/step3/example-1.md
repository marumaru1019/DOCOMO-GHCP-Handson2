...
3. 変数名は **SBKK** プレフィックスをつける
   例: `SBKKCount`, `SBKKValue`
```
...
public class BubbleSort {
    ...
    public void appleSort(int[] SBKKArray) {
        int SBKKN = SBKKArray.length;
        for (int i = 0; i < SBKKN - 1; i++) {
            for (int j = 0; j < SBKKN - i - 1; j++) {
                if (SBKKArray[j] > SBKKArray[j + 1]) {
                    // swap SBKKArray[j] and SBKKArray[j+1]
                    int SBKKTemp = SBKKArray[j];
                    SBKKArray[j] = SBKKArray[j + 1];
                    SBKKArray[j + 1] = SBKKTemp;
                }
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        int[] SBKKArray = {64, 34, 25, 12, 22, 11, 90};
        sorter.appleSort(SBKKArray);
        System.out.println("Sorted array:");
        for (int SBKKValue : SBKKArray) {
            System.out.print(SBKKValue + " ");
        }
    }
}
```

Copilot は**規約ファイルの内容**（クラス冒頭コメント、メソッド名ルール、変数名のSBKKプレフィックス等）を反映した形でコードを生成してくれます。
